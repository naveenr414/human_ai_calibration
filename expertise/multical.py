...
def compute_mse_cluster(train_dataframe,test_dataframe,human_column,reference_column):
    """Compute the accuracy for a dataframe between
        the maximum predictor from a human column -> reference column"""
...
def assess_callibration_no_split(dataframe,human_column,cluster_column,reference_column):
    """Assess a given multicallibration = clustering in a given dataframe
    ...
    train_dataframe = test_dataframe = dataframe
...
def assess_callibration_train_test(train_dataframe,test_dataframe,human_column,cluster_column,reference_column):
    """Assess a given multicallibration = clustering in a given dataframe
    
    Arguments:
        train_dataframe: Pandas Dataframe with all the information
        test_dataframe: Pandas Dataframe with all the information
        human_column: String, which column contains human assessments
        cluster_column: Which column contains the clusters we use
        reference_column: True label column"""

    train_dataframe[reference_column] = pd.to_numeric(train_dataframe[reference_column], errors='coerce')
    train_dataframe[human_column] = pd.to_numeric(train_dataframe[human_column], errors='coerce')

    test_dataframe[reference_column] = pd.to_numeric(test_dataframe[reference_column], errors='coerce')
    test_dataframe[human_column] = pd.to_numeric(test_dataframe[human_column], errors='coerce')
    ...
    for val in cluster_vals:
        clean_train_data = train_dataframe[train_dataframe[cluster_column]==val][[reference_column, human_column]].dropna()
        clean_test_data = test_dataframe[test_dataframe[cluster_column]==val][[reference_column, human_column]].dropna()
        avg_acc += len(clean_test_data)/len(test_dataframe)*compute_mse_cluster(clean_train_data,clean_test_data,human_column,reference_column)
